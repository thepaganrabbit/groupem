...
const CreateGroupModal = ({action, state}: Interactions): React.JSX.Element => {
  ...
  const [filterableContacts, setFilterableContacts] =
    React.useState<Contact[]>(contacts);

  const dispatch: Dispatch = useDispatch();
  const isLoading = useSelector(
    ({contacts}: RootModelExt) => contacts.contactsAreLoading,
  );
  ...
  return isLoading && filterableContacts ? (
    <ActivityIndicator size="large" color="#00ff00" />
  ) : (
    <ModalBase state={state} action={action}>
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          ...
          <ScrollView style={styles.scrollView}>
            {filterableContacts &&
              filterableContacts.map((contact: Contact) => (
                <TouchableHighlight
                  key={contact.recordID}
                  activeOpacity={0.6}
                  underlayColor="#dDDDDDD"
                  onPress={() => {
                    const filteredContacts = filterableContacts.filter(
                      (contactFound: Contact) =>
                        contactFound.recordID !== contact.recordID,
                    );
          ...
          <ScrollView style={styles.scrollView}>
            {contactsGathered &&
              contactsGathered.map((contact: Contact) => (
                <Pressable
                  key={contact.recordID}
                  onLongPress={() => {
                    const filteredList = contactsGathered.filter(
                      (allContacts: Contact) =>
                        allContacts.recordID !== contact.recordID,
                    );
