...
class SqDatabase implements ISqDatabase {
  ...
  async createNewGroup({
    group_title,
    contacts,
  }: IntGroupObject): Promise<ExtGroupObject | undefined> {
    try {
      if (!this.db) {
        throw new Error('Db has not been initialized');
      }
      const createGroup: [SQlite.ResultSet] = await this.db?.executeSql(
        `INSERT INTO Groups (group_title, contacts) VALUES ('${group_title}', '${contacts}')`,
      );
      if (!createGroup) throw new Error('Failed to create group.');
      const group: [SQlite.ResultSet] = await this.db?.executeSql(
        `SELECT * FROM Groups WHERE ID = ${createGroup[0].insertId}`,
      );
      if (!group) throw new Error('Failed to find created group.');
      const returnObject = {
        id: group[0].rows.item(0).ID,
        group_title: group[0].rows.item(0).group_title,
        contacts: JSON.parse(group[0].rows.item(0).contacts),
      };
      return returnObject;
    } catch (error) {
  ...
  async getAllGroups(): Promise<ExtGroupObject[] | undefined> {
    try {
      ...
      const groups: [SQlite.ResultSet] = await this.db?.executeSql(
        `SELECT * FROM Groups`,
      );
